...
{0x0F7B, 0x0F08, 151, 651,100,"bloodmoss    ",1, "normal_agility   "},
{0x0F7B, 0x0F08, 351, 851,100,"bloodmoss    ",3, "greater_agility  "},
{0x0F84, 0x0F07, 0, 500,100,"garlic       ",1, "lesser_cure      "},
{0x0F84, 0x0F07, 251, 751,100,"garlic       ",3, "normal_cure      "},
{0x0F84, 0x0F07, 651,1151, 70,"garlic       ",6, "greater_cure     "},
{0x0F8C, 0x0F0D, 51, 551,100,"sulfurous_ash",3, "lesser_explosion "},
{0x0F8C, 0x0F0D, 351, 851,100,"sulfurous_ash",5, "normal_explosion "},
{0x0F8C, 0x0F0D, 651,1151, 70,"sulfurous_ash",10, "greater_explosion"},
{0x0F85, 0x0F0C, 0, 500,100,"ginseng      ",1, "lesser_healing   "},
{0x0F85, 0x0F0C, 151, 651,100,"ginseng      ",3, "normal_healing   "},
{0x0F85, 0x0F0C, 551,1051, 90,"ginseng      ",7, "greater_healing  "},
{0x0F8D, 0x0F06, 0, 500,100,"spiders_silk ",1, "normal_nightsight"},
{0x0F88, 0x0F0A, 0, 500,100,"nightshade   ",1, "lesser_poison    "},
{0x0F88, 0x0F0A, 151, 651,100,"nightshade   ",2, "normal_poison    "},
{0x0F88, 0x0F0A, 551,1051, 90,"nightshade   ",4, "greater_poison   "},
...
public __nxw_sk_alchemy( const itm, const chr)
{
	...
	{
		chr_message( chr, _,"It must be inside your backpack");
		return;
	}
	alch_show(chr, 1);
}
...
public alch_show(const chr, const pagenumber)
{
	...
	for( new i=k;i<j;++i)
	{
		gui_addText(alch_menu,309,76,33,"Amount:");
...
public createPotion(const chr, const arrayline, const amount)
{
	...
	{
		chr_message(chr, _, "You have too few empty bottles!");
		return;
	}
	...
	if((reagnumber >= potionProperty[arrayline][reagentneed]) && (alchskill >= potionProperty[arrayline][alchemmin]))
	{
		...
		new emotetext[50];
		sprintf(emotetext, "*%s starts grinding some %s in the mortar.*", chrname, potionreg); //combine the names
	...
	{
		chr_message(chr, _, "Your skill is not high enough");
		return
	}
	else if(alchskill >= potionProperty[arrayline][alchemmin])
	{
		chr_message(chr, _, "You have not enough reagents");
		return		
	}
	else chr_message(chr, _, "Your skill is too low and you have not enough reagents");
}