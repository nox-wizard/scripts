...
{
	chr_message( chr, _, msg_sk_evalintDef[0] );
...
public __evintTarget( const t, const cc, const target, const x, const y, const z, const model, const param1 )
{
	...
	if (target < 0) {
		chr_message( cc, _, msg_sk_evalintDef[1]);
		return;
	}

	if (cc == target) {
		chr_message( cc, _, msg_sk_evalintDef[2]);
		return;
	}
	if (chr_distance(cc, target) >= 10) {
		chr_message( cc, _, msg_sk_evalintDef[3]);
		return;
	}
	if (!chr_checkSkill(cc, SK_EVALUATINGINTEL, 0, 1000)) {
        chr_message( cc, _, msg_sk_evalintDef[4]);
        return;
    }
	...
	if (int > 100) strunpack(intDes, msg_sk_evalintDef[5]);
		...
		switch(int)
		{
			...
			case 91..100:
				strunpack(intDes, msg_sk_evalintDef[15]);
			default:
	...
	{
		case 0..9:
			chr_message( cc, _, msg_sk_evalintDef[18], prc);
 		case 10..19:
			chr_message( cc, _, msg_sk_evalintDef[19], prc);
		case 20..29:
			chr_message( cc, _, msg_sk_evalintDef[20], prc);
		case 30..39:
			chr_message( cc, _, msg_sk_evalintDef[21], prc);
		case 40..49:
			chr_message( cc, _, msg_sk_evalintDef[22], prc);
		case 50..59:
			chr_message( cc, _, msg_sk_evalintDef[23], prc);
		case 60..69:
			chr_message( cc, _, msg_sk_evalintDef[24], prc);
		case 70..79:
			chr_message( cc, _, msg_sk_evalintDef[25], prc);
		case 80..89:
			chr_message( cc, _, msg_sk_evalintDef[26], prc);
		case 90..99:
			chr_message( cc, _, msg_sk_evalintDef[27], prc);
		case 100:
			chr_message( cc, _, msg_sk_evalintDef[28], prc);
		default:
